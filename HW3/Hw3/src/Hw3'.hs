...
bstInsert k v tree = balance (bstInsert' k v tree)
...
bstDelete k tree = balance (bstDelete' k tree)
...
bstDelete' k (Bind k' v l r)
         ...
         | otherwise = case l of
                         Emp -> r
                         _   -> Bind key val (bstDelete key l) r
                             where
                             (key, val)              = findIn l
...
genBal = toBalanced . nubBy eqMethod . sortBy comMethod <$> listOf genBSTadd
  where
    comMethod (BSTadd k _) (BSTadd k' _) = compare k k'
    eqMethod (BSTadd k _) (BSTadd k' _)  = k == k'
    toBalanced [] = Emp
    toBalanced xs = let (xs1, (BSTadd k v):xs2) = splitAt (length xs `div` 2) xs
                        l                       = toBalanced xs1
                        r                       = toBalanced xs2
                    in Bind k v l r

-- Why not working?
--toBalanced :: [BSTadd k v] -> BST k v
--toBalanced []   = Emp
--toBalanced xs = let (xs1, (BSTadd k v):xs2) = splitAt (length xs `div` 2) xs
--                    l                       = toBalanced xs1
--                    r                       = toBalanced xs2
...
-- TODO
-- implementing an AVL tree, 45 mins
...
balance Emp                 = Emp
balance tree@(Bind k v l r)
  | balFactor tree > 1  && balFactor l < 0 = leftRightRotate tree
  | balFactor tree > 1                     = leftLeftRotate tree
  | balFactor tree < -1 && balFactor r > 0 = rightLeftRotate tree
  | balFactor tree < -1                    = rightRightRotate tree
  | otherwise                              = tree
  where
    leftRightRotate  (Bind k v (Bind k' v' l' (Bind k'' v'' l'' r'')) r) = (Bind k'' v'' (Bind k' v' l' l'') (Bind k v r'' r))
    leftLeftRotate   (Bind k v (Bind k' v' l' r') r)                     = (Bind k' v' l' (Bind k v r' r))
    rightLeftRotate  (Bind k v l (Bind k' v' (Bind k'' v'' l'' r'') r')) = (Bind k'' v'' (Bind k v l l'') (Bind k' v' r'' r'))
    rightRightRotate (Bind k v l (Bind k' v' l' r'))                     = (Bind k' v' (Bind k v l l') r')

balFactor :: (Num a, Ord a) => BST k v -> a
balFactor Emp            = 0
balFactor (Bind k v l r) = height l - height r
...
prop_bitSubtractor_Correct cin xs= 
  binary (sampleN out) == max 0 (binary xs - binary (sample1 cin))
  where (out, _) = bitSubtractor (cin, map lift0 xs)
...
bitSubtractor (cin, x:xs)  = ((and2 (out, snot cout)):outs, cout)
        where (out, c)     = halfsub (x, cin)
              (outs, cout) = bitSubtractor (c, xs)

snot :: Signal -> Signal
snot = lift1 $ \x -> not x

greater2 ::  (Signal, Signal) -> Signal
greater2 = lift2 $ \x y -> x > y

halfsub :: (Signal, Signal) -> (Signal, Signal)
halfsub (x,y) = (out, cout)
  where out   = xor2 (x, y)
        cout  = greater2 (y, x)
...
prop_Multiplier_Correct xs ys=
  binary (sampleN out) == (binary xs) * (binary ys)
  where out = multiplier (map lift0 xs, map lift0 ys)
...
multiplier ([], ys)   = map (const low) ys
