...
evalS (Assign var exp) = do
                           s <- get
                           val <- evalE exp
                           put $ Map.insert var val s
                           --return () -- needed?

evalS (If exp s1 s2)   = do
                           val <- evalE exp
                           case val of
                             BoolVal True  -> evalS s1
                             BoolVal False -> evalS s2
                             _             -> throwError (IntVal (-1))

evalS (While exp s)    = evalS $ If exp (Sequence s $ While exp s) Skip
...
evalS (Print s exp)    = evalE exp >>= \val -> tell $ s ++ show val ++ "\n"

evalS (Throw exp)      = evalE exp >>= throwError

evalS (Try s1 var s2)  = evalS s1 `catchError` \v -> evalS $ Sequence (Assign var (Val v)) s2
...
evalE (Op bop e1 e2) = do
                         ...
                         evalOp bop v1 v2
                           where
                             ...
                             evalOp Divide (IntVal v1) (IntVal v2) =
                               if v2 == 0 then throwError (IntVal 1) else return (IntVal (v1 `div` v2)) -- if denominator is zero
                             ...
                             evalOp _ _ _                          = throwError (IntVal 2) -- if the operator or operands are invalid
...
--runEval act sto = flip runState sto (runWriterT $ runErrorT $ act)
...
-- *Hw3 Hw3> let st = Map.empty
-- *Hw3 Hw3> execute st testprog1
...
-- *Hw3 Hw3> let st = Map.empty
-- *Hw3 Hw3> execute st testprog1
-- (fromList [("A",IntVal 100),("E",IntVal 1),("X",IntVal 0),("Y",IntVal 1),("Z",IntVal 101)],Nothing,"")
...
bstInsert k v t = balance (bstInsert' k v t)
...
bstDelete k t = balance (bstDelete' k t)
...
bstDelete' k (Bind k' v l r)
     ...
     | otherwise = case maxRight l of
                     Nothing         -> r
                     Just (l', k, v) -> Bind k v l' r


maxRight :: (Ord k) => BST k v -> Maybe (BST k v, k, v)
maxRight Emp              = Nothing
maxRight (Bind k v l Emp) = Just (l, k, v)
maxRight (Bind k v l r)   = case maxRight r of
                              Nothing          -> Nothing
                              Just (t, k', v') -> Just (Bind k v l t, k', v')
...
genBal = liftM ofBSTops treeListGen
  where
    treeListGen = listOf genBSTadd
...
genBal' = mkBalanced . nubBy eqKeys . sortBy cmpKeys <$> listOf genBSTadd
  where
    ...
    mkBalanced [] = Emp
    mkBalanced [BSTadd k v] = Bind k v Emp Emp
...
balance Emp                 = Emp
balance tree@(Bind k v l r) = doRotate (heightDiff tree)
  where
    doRotate n
      | n > 1 && heightDiff l < 0  = rotateRightSubTree (Bind k v (rotateLeftSubTree l) r)
      | n > 1                      = rotateRightSubTree tree
      | n < -1 && heightDiff r > 0 = rotateLeftSubTree (Bind k v l (rotateRightSubTree r))
      | n < -1                     = rotateLeftSubTree tree
      | otherwise                  = tree

heightDiff :: (Num a, Ord a) => BST k v -> a
heightDiff Emp            = 0
heightDiff (Bind k v l r) = height l - height r

rotateLeftSubTree :: BST k v -> BST k v
rotateLeftSubTree Emp                 = Emp
rotateLeftSubTree tree@(Bind k v l r) = case r of
                                          Emp                -> tree
                                          (Bind k' v' l' r') -> Bind k' v' (Bind k v l l') r'

rotateRightSubTree :: BST k v -> BST k v
rotateRightSubTree Emp                 = Emp
rotateRightSubTree tree@(Bind k v l r) = case l of
                                           Emp                -> tree
...
prop_bitSubtractor_Correct l1 l2 = binary (sampleN sum) == max 0 (binary l2 - binary (sample1 l1))
  where
    (sum, cout) = bitSubtractor (l1, map lift0 l2)
...
bitSubtractor (bin, x:xs) = ((and2 (diff, hxor bout)):diffs, bout)
  where
...
test3 = probe [("cin",bin), ("in1",in1), ("in2",in2), ("in3",in3), ("in4",in4),
               ("  s1",s1), ("s2",s2), ("s3",s3), ("s4",s4), ("c",c)]
  where
    bin = high
    in1 = high
...
prop_Multiplier_Correct l1 l2 = (binary l1 * binary l2) == (binary $ sampleN $ multiplier (map lift0 l1, map lift0 l2))
